# OpenCode Jacket Architecture

## Design principle: Predictable > Clever

OpenCode Jacket optimizes for **predictability** over "clever" auto-detection. Users should be able to form a correct mental model of what will happen.
...
- OpenCode Jacket runs on a desktop/laptop and **starts** OpenCode locally.
- The OpenCode server runs on loopback (default `127.0.0.1:4096`).
- OpenCode Jacket UI connects via the official SDK and listens to events.

## OpenCode Integration (Exact SDK + APIs)

OpenCode Jacket uses the official JavaScript/TypeScript SDK:
...
OpenCode Jacket must be real-time. It subscribes to SSE events:
...
OpenCode Jacket maps a "Task Run" to an OpenCode **Session**.
...
OpenCode Jacket's file browser and "what changed" UI are powered by:
...
OpenCode Jacket must surface permission requests clearly and respond explicitly.

- Permission response API:
  - `client.permission.reply({ requestID, reply })` (where `reply` is `once` | `always` | `reject`)

OpenCode Jacket UI should:
...
OpenCode Jacket's settings pages use:
...
OpenCode Jacket exposes two extension surfaces:
...
- OpenCode Jacket server exposes `POST /workspace/:id/engine/reload`.
...
- Reload requests follow OpenCode Jacket server approval rules.
...
OpenCode Jacket conceptually treats "workspace" as the current project/path.

## Folder Authorization Model

OpenCode Jacket enforces folder access through **two layers**:

1. **OpenCode Jacket UI authorization**
   - user explicitly selects allowed folders via native picker
   - OpenCode Jacket remembers allowed roots per profile

2. **OpenCode server permissions**
   - OpenCode requests permissions as needed
   - OpenCode Jacket intercepts requests via events and displays them
